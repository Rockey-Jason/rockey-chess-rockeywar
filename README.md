...
- A committed move uses exactly one temporary action clone: it starts at the source cell, travels to the destination, then is removed. The destination piece is hidden during that animation, so no stationary duplicate remains at the source.
